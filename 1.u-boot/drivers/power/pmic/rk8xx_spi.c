...
static int _spi_write(struct udevice *dev, uint reg, const u8 *buffer, int len)
{
	...
	if (len < 1) {
		printf("rk806 write error: len < 1\n");
		return -EINVAL;
	}
			  ...
			  int len)
{
	...
	if (ret)
		printf("rk806 read reg(0x%x) error: %d\n", reg, ret);
			   ...
			   int len)
{
	...
	if (ret)
		printf("rk806 write reg(0x%x) error: %d\n", reg, ret);
...
static int rk8xx_spi_bind(struct udevice *dev)
{
	...
	children = pmic_bind_children(dev, dev->node_, power_key_info);
	if (!children)
		debug("%s: %s - no child found\n", __func__, dev->name);
...
static int rk8xx_spi_probe(struct udevice *dev)
{
	...
	if (spi->seq_ < 0) {
		printf("Failed to configure the spi num\n");
		return -EINVAL;
	}

	slave = spi_setup_slave(spi->seq_, plat->cs, plat->max_hz,
				plat->mode);
	if (!slave)
		return -ENODEV;
	priv->slave = slave;
	...
	if (ret) {
		printf("rk806 name read error: %d\n", ret);
		return ret;
	}

	ret = rk806_spi_read(dev, RK806_CHIP_VER, &lsb, 1);
	if (ret) {
		printf("rk806 version read error: %d\n", ret);
		return ret;
	}

	priv->variant = ((msb << 8) | lsb) & RK8XX_ID_MSK;
	printf("spi%d: RK%x%x: %d\n", spi->seq_, msb, (lsb >> 4), lsb & 0x0f);
	...
	if ((!strcmp(dev->name, "rk806master@0")) || (!strcmp(dev->name, "rk806slave@1"))) {
		if (value != HW_DUAL_PMIC) {
			printf("HW single pmic, the firmware dual pmic(0x%x)!\n", value);
			//run_command("download", 0);
		}
	} else {
		if (value != HW_SINGLE_PMIC) {
			printf("HW dual pmic, the firmware single pmic(0x%x)!\n", value);
			//run_command("download", 0);
		}
	}
	...
	if ((lsb & 0x0f) == VERSION_AB) {
		...
		if (ret) {
			printf("rk806 RK806_SYS_CFG1 read error: %d\n", ret);
			return ret;
		}
		value |= 0x80;
	...
	priv->rst_fun = dev_read_u32_default(dev, "pmic-reset-func", 0);
	printf("pmic: reset func=0x%d\n", priv->rst_fun);
